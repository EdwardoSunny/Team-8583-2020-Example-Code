...
public class SynchronousPIDF
{
     ...
     * @param isFirstCall
     *            whether this call is the first call of the function, in which case deltaTime would be ignored
     */
    public synchronized double calculate(double input, double deltaTime, boolean isFirstCall)
    {
        ...
        error = isContinuous ? Util.boundToScope(input - setpoint, minInput, maxInput)
        ...
        output = Util.limit(Util.conditionalInvert(
                -(kp * proportionalError + ki * accumulativeError + kd * differentialError + kf * setpoint),
                isInverted), minOutput, maxOutput);
        previousError = error;
        return output;
    }
    ...
    public synchronized void setInputRange(double minInput, double maxInput)
    {
        ...
        {
            // throw new IllegalArgumentException("minInput cannot be greater than maxInput");
            return;
        }
    ...
    public synchronized void setOutputRange(double minOutput, double maxOutput)
    {
        ...
        {
            // throw new IllegalArgumentException("minOutput cannot be greater than maxOutput");
            return;
        }
