...
public class Util
{
    ...
    {
        return epsilonEquals(lhs, rhs, Math.ulp(Math.max(lhs, rhs)));
    }
    ...
    public static int boundToScope(int val, int scopeFloor, int scopeCeiling)
    {
        ...
        val %= scopeSpan;
        val += val >= 0.0 ? scopeFloor - scopeFloor % scopeSpan : scopeCeiling - scopeCeiling % scopeSpan;
        if (val < scopeFloor)
        {
            val += scopeSpan;
        }
    ...
    public static double boundAngleToClosestScopeWithinLimit(double angle, double referenceAngle, double minAngle,
            double maxAngle)
    {
    ...
    public static double applyRemappedDeadband(double val, double deadband)
    {
        ...
        return (Math.abs(val) > absDeadband) ? (val - Math.copySign(absDeadband, val)) / (1 - absDeadband) : 0.0;
    }
    ...
    {
        double absDeadband = Math.abs(deadband);
        return input.norm() > absDeadband ? input.translateBy(Translation2d.fromPolar(input.direction(), -absDeadband))
                .scale(1 / (1 - absDeadband)) : Translation2d.identity();
    }
