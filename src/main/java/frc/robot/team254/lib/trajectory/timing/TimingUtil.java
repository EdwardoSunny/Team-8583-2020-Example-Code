...
public class TimingUtil
{
    public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(boolean reverse,
            final DistanceView<S> distance_view, double step_size, final List<TimingConstraint<S>> constraints,
            double start_velocity, double end_velocity, double max_velocity, double max_abs_acceleration,
            double max_deceleration, int slowdown_chunks)
    {
        ...
        return timeParameterizeTrajectory(reverse, states, constraints, start_velocity, end_velocity, max_velocity,
                max_abs_acceleration, max_deceleration, slowdown_chunks);
    }

    public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(boolean reverse,
            final List<S> states, final List<TimingConstraint<S>> constraints, double start_velocity,
            double end_velocity, double max_velocity, double max_abs_acceleration, double max_deceleration,
            int slowdown_chunks)
    {
        ...
        for (int i = 0; i < states.size(); ++i)
        {
            ...
            while (true)
            {
                ...
                // vf = sqrt(vi^2 + 2*a*d)
                constraint_state.max_velocity = Math.min(max_velocity, Math.sqrt(
                        predecessor.max_velocity * predecessor.max_velocity + 2.0 * predecessor.max_acceleration * ds));
        ...
        for (int i = states.size() - 1; i >= 0; --i)
        {
            ...
            while (true)
            {
                ...
                final double new_max_velocity = Math
                        .sqrt(successor.max_velocity * successor.max_velocity + 2.0 * successor.min_acceleration * ds);
        ...
        for (int i = 0; i < states.size(); ++i)
        {
            ...
            if ((Double.isNaN(accel) || Math.abs(accel) <= kEpsilon) && (Double.isNaN(v) || Math.abs(v) <= kEpsilon)
                    && i > 0)
            {
            ...
            if (i > 0)
            {
                ...
                {
                    System.out.println("Path generation error --- i: " + i + " v: " + v + " accel: " + accel + " ds: "
                            + ds + " state list size: " + states.size());
                    throw new RuntimeException();
                }
            }
            t += dt;
